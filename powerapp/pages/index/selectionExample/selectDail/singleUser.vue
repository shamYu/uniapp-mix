			         ...
			         <view class="uni-list-cell uni-collapse" :class="'listBg'+index">
			             ...
			             <view class="uni-collapse-content" :class="list.show ? 'uni-active' : ''">
			 				<view v-if="list.type === 'swiper'">
			 					<ul class="muneFirst">
									<li v-for="(item,index1) in nameList" :key="index1" @click="lisigleEvt(index1,item.name)" :class="actIndex==index1?'on':''">{{item.name}}</li>
								</ul>
			 				</view>
...
<script>
	...
	export default {
		...
		created (){
			...
			let suser = this.$pdStorage.get("su");
			if(suser){
				for(let i=0;i<this.nameList.length;i++){
					if(this.nameList[i].name == suser){
						this.actIndex = i;
						this.checkItem = suser;
						break;
					}
				}
			}
		...
		methods:{
			...
			showlist (){
				//跳转页面并返回上一页传值
				this.$pdView.passValToPrePage({suData:String(this.checkItem)},1)
			},
			deletelist (){
				this.actIndex = -1;
				this.checkItem = "";
