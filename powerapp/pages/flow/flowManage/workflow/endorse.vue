	...
	<view class="flowContent">
		...
		<view>
			<view class="table-list-ctn">
				...
				<block>
					<ul class="opt-box users" v-if="!isSerial">
						<li>
							<h1>流转后步骤：</h1>
							<view style="width:90%;word-break: break-all;word-wrap: break-word;line-height: 90upx;">
								<radio-group class="uni-flex" name="status">
									<label>
										<radio value="FINISH" :checked="bean.status == 'FINISH' ? true : false" v-model="bean.status"/>结束</label>
									<label>
										<radio value="WAIT" :checked="bean.status == 'WAIT' ? true : false" v-model="bean.status"/>等待</label>
								</radio-group>
							</view>
						</li>
						<li>
					...
					<ul class="opt-box users" v-else>
						<li>
							<h1>下步办理人：</h1>
							<i v-for="user in nextProcessorUsers" :key="user.userId">{{user.userName}}</i>
						</li>
					</ul>
				</block>
			</view>
	...
	export default {
		...
		methods: {
			...
			loadWorkflowProBeforeInfo(e){
				this.$flowManageSvr.loadWorkflowProBeforeInfo(e.stepId,"ENDORSE").then(res=>{
					this.workflow = res.data_json;
					//===========数据初始化===========
					this.kjyjArray = this.workflow.yclInfo.kjyjData;
					//如果是串行，则直接设置下步办理人
					if(this.workflow.yclInfo.isSerial){
						this.isSerial = true;
						this.bean.selectUsers = this.workflow.yclInfo.nextProcessor.userId;
						this.nextProcessorUsers.push(this.workflow.yclInfo.nextProcessor);
					}else{
						this.bean.status = "FINISH";
						this.bean.activePoint = "ALL";
					}
					//有意见类型，默认选择第一项
			...
			endorse(){
				...
				this.$pdAction.showWait("正在发起加签，请稍等！",false);
				this.$flowManageSvr.workflowEndorse(this.bean).then(res=>{
					this.$pdAction.closeWait();
					this.$pdAction.alert("加签成功",{title:"操作结果"}).then(res=>{
					...
					this.$pdAction.closeWait();
					this.$pdAction.toast("加签失败!"+error,{icon:'none'});
				})
			},
			showKjyj(){
