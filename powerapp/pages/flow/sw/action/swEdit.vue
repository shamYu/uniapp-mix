<template>
    <view class="page">
		<uni-nav-bar class="nav_custom" fixed="true" left-icon="back" color="#ffffff" @click-left="back" @click-right="save" right-text="保存"
		 title="修改申请信息"></uni-nav-bar>
        <view class="editLayer">
            <view>
                <block>
                    <view class="uni-collapse listBg1" style="background:#FFFFFF;">
		...
		methods:{
			...
			queryBean(e){
				this.$pdResource.service({
					...
					this.LWLXMC = this.formatDmjbh(this.bean.LWLX,'LWLX');
					this.LWWZMC = this.formatDmjbh(this.bean.XTXH,'LWWZ');
					this.MJMC = this.formatDmjbh(this.bean.MJ,'MJ');
					this.JJCDMC = this.formatDmjbh(this.bean.JJCDM,'JJCD');
					this.bean.SWRQ = this.formatDate(this.bean.SWRQ);
					this.FJXX = res.data_fjxx;
					let obj = {
						APPBH: this.bean.XH
					}
