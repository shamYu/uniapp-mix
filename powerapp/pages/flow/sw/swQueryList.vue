			...
			:data-name="{count:'total_count',list:'data_array'}">
				<div class="result-list" v-for="(item,index) in list" :key="index" @tap="openDetail(item)">
					...
					<dd>
						...
						<p><span>接收时间：{{item.SWRQ}}</span></p>
						<p><span>办理期限：{{item.RWQX}}</span></p>
					</dd>
				</div>
			</pd-pullrefresh>
		</view>
	...
	export default {
			...
			data() {
				return {
					...
					top: '95upx',
					type: 'ALL',
					serviceParam:{"searchKey": this.searchKey,"queryDate": this.queryDate,"queryType":'SW'}
				};
			},
			onLoad(e) {
			...
			methods:{
				...
				confirm(e){
					...
					this.serviceParam = {"searchKey": this.searchKey,"queryDate": this.queryDate,"queryType":'SW'};
					this.$refs.pullrefresh1.reachBottom();
				},
				fireQuery(type){
					this.type = type;
				...
				openDetail(item){
					let url = '/powerapp/pages/flow/sw/swDetail?LXDM='+item.LXDM+'&XH='+item.XH+"&BZBH="+item.BZBH+"&TYPE=YB";
